...
{
    public partial class EodrLngMembraneCrossSectionMeasurementForm : Form
    {
        ...
        public EodrLngMembraneCrossSectionMeasurementForm(EodrLngMembraneMenuParameter eodrLngMembraneMenuParameter)
        {
            InitializeComponent();
            _eodrLngMembraneMenuParameter = eodrLngMembraneMenuParameter;
        }

        private void EodrLngMembraneCrossSectionMeasurementForm_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void EodrLngMembraneCrossSectionMeasurementForm_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void EodrLngMembraneCrossSectionMeasurementForm_Load(object sender, EventArgs e)
        {
        ...
        private void BhdPlaneMeasurement()
        {
            ...
            _eodrMeasurementParameter.CuurentFormName = "SHELL MEASUREMENT";
            _eodrMeasurementParameter.MeasurementStartSectionNo = int.Parse(this.numTextBoxMeasurementSectionNo.Text);


            _eodrMeasurementParameter.SetShellSectionNo();
        ...
        private void AddMeasurementData()
        {
            dataSetMeasurementData.DataTableMeasurementData.AddDataTableMeasurementDataRow(
                ...
                GetPlaneCode(_eodrMeasurementParameter.MeasurementPointNo),
                ...
                string.Format("{0,9:##0.00000}", CalculateZCoordinate()),
                "",
                "");
        }

        private string GetPlaneCode(int measurementpointNo)
        {
            if (measurementpointNo < 4) return "SW-P3";
            else if (measurementpointNo < 7) return "SW-P2";
            else if (measurementpointNo < 10) return "SW-P1";
            else if (measurementpointNo < 13) return "TOP";
            else if (measurementpointNo < 16) return "SW-S3";
            else if (measurementpointNo < 19) return "SW-S2";
            else return "SW-S1";
                    }

        private double CalculateZCoordinate()
        {
            double pi = 3.1415926;
            ...
            return double.Parse(_eodrMeasurementParameter.SlopeDistance) * Math.Sin(va) * Math.Sin(ha);
        }
